...
def _normalize_text_response(content, llm_provider: str) -> str:
    # Different LLM SDKs may return None, empty strings, or non-string objects
    # on error or moderation. Validate here to avoid downstream attribute errors.
    if content is None:
    ...
    # Clean <think>...</think> reasoning blocks from thinking models (e.g. DeepSeek R1, MiniMax M3).
    content = _THINK_BLOCK_RE.sub("", content)
    content = _UNCLOSED_THINK_BLOCK_RE.sub("", content).strip()
    if not content:
        raise ValueError(f"[{llm_provider}] returned empty text content")

    # Preserve single and double newlines inside body for paragraph formatting and line reading.
    return content


def _sanitize_error_message(error: object) -> str:
    """
    Sanitize error message returned to WebUI/API to prevent leaking credentials in custom base_urls.
    """
    message = str(error)
...
def _get_response_field(value, key: str):
    """Compatible field accessor for dicts and SDK response objects."""
...
def test_connection() -> tuple[bool, str, float]:
    """
    Test connectivity for the current LLM Provider configuration.
    """
...
def generate_script(
    ...
    for i in range(_max_retries):
        try:
            ...
            if final_script and "quota exceeded" in final_script.lower():
...
#
# Generate social media publishing title, caption, and hashtags based on video script.
# =============================================================================
...
def _normalize_hashtags(raw, count: int) -> List[str]:
    """
    Format and sanitize hashtags returned by LLM into `#tag` list.
    """
...
def generate_social_metadata(
    ...
    """
    Generate social media publishing metadata (title, caption, hashtags).
    """
    platform = _resolve_social_platform(platform)
    language = _normalize_social_language(language)
    video_subject = _limit_social_text(
...
if __name__ == "__main__":
    ...
    script = generate_script(
        video_subject=video_subject, language="en", paragraph_number=1
    )
    print("######################")
    print(script)
