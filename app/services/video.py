...
audio_codec = "aac"
audio_codec = "aac"
# Explicit higher audio bitrate to prevent distortion in containerized environments
audio_bitrate = "192k"
fps = 30
# Small safety margin for video duration to prevent black frames or truncated narration
_VIDEO_DURATION_SAFETY_MARGIN = 0.1
_MIN_MATERIAL_DIMENSION = 480
# Small tolerance allowing dimension rounding (e.g. 478x850 from WhatsApp)
...
def _get_required_video_duration(audio_duration: float) -> float:
    """
    Return target concatenated video duration covering narration audio with safety margin.
    """
...
def is_material_resolution_acceptable(width: int, height: int) -> bool:
    """
    Check whether material resolution is acceptable for synthesis.
    """
...
def _prioritize_unique_source_clips(
    ...
    """
    Prioritize distinct source materials before reusing clips from the same source.
    """
...
def get_ffmpeg_binary():
    """Compatibility wrapper for FFmpeg binary resolver."""
    return utils.get_ffmpeg_binary()


def _get_configured_video_codec() -> str:
    """Read configured video encoder from settings whitelist."""
...
def _ffmpeg_encoder_exists(ffmpeg_binary: str, codec: str) -> bool:
    """Check if current FFmpeg build supports specified encoder."""
    try:
...
def _get_effective_video_codec(preferred_codec: str | None = None) -> str:
    """Return effective video encoder with fallback support."""
...
def _fallback_write_videofile(clip, output_file: str, failed_codec: str, reason: str, **kwargs):
    """Fallback to libx264 when hardware encoding fails."""
...
def _write_videofile_with_codec_fallback(clip, output_file: str, codec: str, **kwargs):
    """Write video with specified codec and automatically fallback to libx264 on failure."""
...
def _format_ffmpeg_concat_path(file_path: str) -> str:
    """Format path for concat demuxer file list."""
...
def concat_video_clips_with_ffmpeg(
    ...
    def run_concat(codec: str):
        ...
        # Use single-pass ffmpeg concatenation to minimize re-encoding degradation
        result = subprocess.run(
            command,
            capture_output=True,
            text=True,
...
def _sanitize_image_file(image_path: str) -> str:
    # Strip corrupted EXIF metadata by re-exporting as clean PNG
...
def _open_image_clip_with_fallback(image_path: str):
    # Try opening image directly, fallback to sanitized copy on error
    try:
...
def _open_video_clip_quietly(video_path: str, audio: bool = False) -> VideoFileClip:
    """
    Open VideoFileClip while suppressing extraneous MoviePy stdout probe logs.
    """
...
def delete_files(files: List[str] | str):
    ...
    # Deduplicate temp file list before unlinking
...
def combine_videos(
    ...
    for video_path in video_paths:
        if not video_path or not os.path.exists(video_path):
            logger.warning(f"combine_videos: skipping missing video file: {video_path}")
            continue
...
def wrap_text(text, max_width, font="Arial", fontsize=60):
    """
    Wrap text to fit within max_width using ImageFont measurements.
    """
...
def _hex_to_rgb(color: str) -> tuple[int, int, int]:
    """Parse #RRGGBB hex color string into RGB tuple."""
...
def _rounded_subtitle_background_clip(
    ...
    """Create rounded translucent subtitle background ImageClip."""
...
def _get_visible_center_position(
    ...
    """
    Compute TextClip center offset using actual non-transparent pixel mask.
    """
...
def _subtitle_font_supports_sample(font_path: str, sample: str) -> bool:
    """Check if font contains glyphs for the sample text."""
    try:
...
def subtitle_font_supports_text(font_path: str, text: str) -> bool:
    """Check if font can render alphanumeric characters in text."""
    sample = "".join(
        dict.fromkeys(
            char
...
def generate_video(
    ...
    """
    Synthesize final video with audio, subtitles, and background music.
    """
